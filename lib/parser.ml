...
and 'a parser = p_state:t -> t option
...
let whitespace_parser ~p_state =
  ...
  | Some Token.Whitespace w -> Some ( add_item ~p_state:(advance p_state) (Token.Whitespace w))
  | _ -> None

let nl_parser ~p_state =
  match current p_state with
  | Some Token.Newline -> Some (add_item ~p_state:(advance p_state) (Token.Newline))
  | _ -> None

let whitespace_or_nl_parser ~p_state =
  attempt ~p_state [whitespace_parser; nl_parser]

(** Parse text. Text is anything that could be seen as a word *)
let text_parser ~p_state =
  match current p_state with
  | Some Token.Text t -> Some (add_item ~p_state:(advance p_state) (Token.Text t))
  | _ -> None

(** Parse emphasis char *)
let emph_char_parser ~p_state =
  match current p_state with
  | Some Token.Star -> Some (add_item ~p_state:(advance p_state) Token.Star)
  | Some Token.Slash -> Some (add_item ~p_state:(advance p_state) Token.Slash)
  | Some Token.Tilde -> Some (add_item ~p_state:(advance p_state) Token.Tilde)
  | Some Token.Underscore -> Some (add_item ~p_state:(advance p_state) Token.Underscore)
  | Some Token.Backtick -> Some (add_item ~p_state:(advance p_state) Token.Backtick)
  | _ -> None
...
let rec start_emph_parser ~p_state =
  let p_state, tk = (
      match current p_state with
      | Some Token.Star -> (emph_char_parser ~p_state:(start_node Green.Bold ~p_state), Token.Star)
      | Some Token.Slash -> (emph_char_parser ~p_state:(start_node Green.Italics ~p_state), Token.Slash)
      | Some Token.Tilde -> (emph_char_parser ~p_state:(start_node Green.StrikeThrough ~p_state), Token.Tilde)
      | Some Token.Underscore -> (emph_char_parser ~p_state:(start_node Green.Underline ~p_state), Token.Underscore)
      | Some Token.Backtick -> (emph_char_parser ~p_state:(start_node Green.Monospace ~p_state), Token.Backtick)
      | _ -> (None, Token.AtAt)
    )
  in
...
let rec end_emph_parser ~p_state tk =
  ...
  | Some p_state ->
     (* If we've found the emphasis we're looking for we keep parsing
        ...
        the state *)
     if tk == tk2 then
       match end_emph_parser ~p_state tk with
       | Some _ -> Some p_state
...
let rec contents_parser ?(exit=fun ~p_state:_ -> None) ~p_state () =
  let continue p_state =
    match as_text_parser ~p_state with
    | Some p_state -> contents_parser ~exit ~p_state ()
    | None -> p_state
  in
  ...
  | Some Token.Whitespace w ->
     (match emph_text_parser
              ~p_state:(add_item ~p_state:(advance p_state) (Token.Whitespace w))
      with
      | Some p_state -> contents_parser ~exit ~p_state ()
     ...
     | None ->
        ...
        | Some Token.Newline ->
           ...
           (match current p_state with
            (* Unordered list *)
            | Some Token.Dash ->
               (match ulist_parser ~p_state:(start_node Green.UList ~p_state) with
               | Some p_state -> contents_parser ~exit ~p_state:(end_node ~p_state) ()
               | None -> contents_parser ~exit ~p_state ())
            | _ -> continue p_state)
        | _ -> continue p_state
...
and emphasis_stack_parser ~p_state = function
  ...
  | hd :: tl ->
     ...
     | None -> 
        end_node ~p_state:(contents_parser
          ~p_state
          ~exit:(
            fun ~p_state ->
...
and as_text_parser ~p_state =
  match current p_state with
  | Some tk -> Some (add_item ~p_state:(advance p_state) (tk))
  | None -> None
...
and link_parser ~p_state =
  ...
  let rec link_url_parser ~p_state =
    ...
    | Some Token.RSquareB -> (add_item ~p_state:(advance p_state) Token.RSquareB)
    (* If we run into a LSquareB we go down two levels so we have
       to exit twice. This balances the brackets *)
    | Some Token.LSquareB ->
       link_url_parser ~p_state:
         (link_url_parser ~p_state:(add_item ~p_state:(advance p_state) Token.LSquareB))
    | _ ->
       match as_text_parser ~p_state with
  ...
  let rec link_name_parser ~p_state =
    ...
    let exit ~p_state =
      ...
      | Some Token.LParen ->
         Some (link_name_parser ~p_state:(
                   link_name_parser ~p_state:(
                       advance (add_item ~p_state Token.LParen))))
      (* If we run into the RParen we exit *)
      | Some Token.RParen ->
         Some (p_state |> add_item Token.RParen |> advance)
      | _ -> None
    in
    contents_parser
      ~p_state
  ...
  let close_link p_state =
    match current p_state with
    | Some Token.RSquareB ->
       Some (add_item ~p_state Token.RSquareB |> advance |> end_node)
    | _ -> None
  in

  (* Start link parsing *)
  let p_state = start_node ~p_state Green.Link in
...
let init_content_parser ~p_state =
  ...
  | Some Token.Dash ->
     (match ulist_parser ~p_state:(start_node Green.UList ~p_state) with
      | Some p_state -> contents_parser ~p_state:(end_node ~p_state) ()
...
let parse tokens =
  let p_state = init_content_parser ~p_state:{tokens = tokens; cursor = 0; ast = Green.create }
  in
