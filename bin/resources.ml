  ...
  | Unix_error (e, _, _) ->
      Error
        (K_error.FileIOFailure
           ("Failed to create runtime directory: " ^ error_message e))

(** Create socket *)
let create_socket () =
  let open Lwt_unix in
  try Ok (socket PF_UNIX SOCK_STREAM 0)
  with Unix_error (e, _, _) ->
    Error
      (K_error.SocketCreationFailure
         ("Failed to create socket: " ^ error_message e))
  ...
  | Unix_error (EADDRINUSE, _, _) -> (
      let%lwt () =
        Logs_lwt.debug (fun m ->
            m "Socket already exists. Deleting and trying again.")
      in
      try%lwt
      ...
      with Unix_error (e, _, _) ->
        return_error
        @@ K_error.SocketCreationFailure
             ("Failed to bind to socket: " ^ error_message e))
  | Unix_error (e, _, _) ->
      return_error
      @@ K_error.SocketCreationFailure
           ("Failed to bind socket: " ^ error_message e)
