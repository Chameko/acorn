...
let handle_message msg =
  ...
  | "inc" ->
      counter := !counter + 1;
      ("Counter has been incramented", true, true)
...
let rec handle_connection ic oc () =
  try%lwt
    ...
    | Some msg ->
        ...
        if cont then handle_connection ic oc () else return_ok term
    | None ->
...
let is_server_running paths =
  ...
  let read_pid_file () =
    In_channel.with_file (Paths.pid_file paths) ~f:(fun ic ->
        try Ok (Stdlib.String.trim @@ In_channel.input_all ic)
        with Sys_error e ->
  ...
  let pid =
    ...
    with Sys_error e -> (
      ...
      try Result.(create_pid paths >>= read_pid_file)
      with Sys_error e ->
...
let rec serve_loop sock () =
  ...
  with Unix.Unix_error (e, _, _) ->
    return_error
    @@ K_error.SocketIOFailure
         ("Failed to accept connection: " ^ Unix.error_message e)
...
let start_server paths =
  ...
  | Ok () -> (
      ...
      | Ok true ->
          Logs.err (fun m ->
              m "%s" @@ K_error.output K_error.ServerAlreadyRunning)
      (* Error  *)
      | Error e -> Logs.err (fun m -> m "%s" @@ K_error.output e)
