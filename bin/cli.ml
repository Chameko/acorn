...
let start init kakoune server request features =
  ...
  | None -> (
      ...
      | Ok paths ->
          if server then start_server paths
          else Lwt_main.run (start_client paths))
...
let features =
  ...
  Arg.(
    value
    & opt (list string) [ "all" ]
    & info [ "f"; "features" ] ~docv:"FEATURES" ~doc)
...
let cmd =
  ...
  let man =
    [
      ...
      `P
        "$(tname) enables advanced functionality for acorn buffers in kakoune. \
         To use\n\
